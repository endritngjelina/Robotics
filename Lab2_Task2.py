            ...
            if diff_x < 0 and diff_y < 0:
                ...
                if map_angle >180 and map_angle < 270 and Waypoint == 0 and totaltime >= 40:
                    ...
                    while robot.step(timestep) != -1:
                        ...
                        Pose_x , Pose_y = updatePoseCircular(imu.getRollPitchYaw()[2], Pose_x, Pose_y,Rads)
                    
                        pose_angle = imu_cleaner(imu.getRollPitchYaw()[2])
                        
                        #print("Pose at circular 4 at time " + str(totaltime) + " is :" + str(Pose_x) +" , "+str(Pose_y) +" , "+str(round(pose_angle,0)))
                          
                        if round(totaltime,1) == T[m]:
                            print("Pose at time " + str(T[m]) + " is :" + str(Pose_x) +" , "+str(Pose_y) +" , "+str(round(pose_angle,0)))
                ...
                if  map_angle > 180 and map_angle <270 and Waypoint == 1:
                    ...
                    while robot.step(timestep) != -1:
                       ...
                       # print("Pose at circular 3 time " + str(T[m]) + " is :" + str(Pose_x) +" , "+str(Pose_y) +" , "+str(round(pose_angle,0)))
                ...
                if map_angle >180 and map_angle < 270 and Waypoint == 0 and totaltime <= 40:
                    ...
                    while robot.step(timestep) != -1:
                      ...
                      #  print("Pose at time " + str(totaltime) + " is :" + str(Pose_x) +" , "+str(Pose_y) +" , "+str(round(pose_angle,0)))
                ...
                if start_x ==-5 and start_y == -15:
                    ...
                    while robot.step(timestep) != -1:
                     #   print("Time: " +str(robot.getTime()))
                       ...
                       # print("Pose_x rotation is " + str(Pose_x))
                       # print("Pose_y rotation is " + str(Pose_y))
              
                        pose_angle = imu_cleaner(imu.getRollPitchYaw()[2])
                        if pose_angle > 359:
                            pose_angle = 360- pose_angle
                      #  print("Pose at time " + str(totaltime) + " is :" + str(Pose_x) +" , "+str(Pose_y) +" , "+str(round(pose_angle,0)))
            ...
            while robot.step(timestep) != -1:
             ...
             #   print("Time: " +str(robot.getTime()))
               ...
               # print("totaltime is " + str(totaltime))
                ...
                Pose_x , Pose_y = updatePose(imu.getRollPitchYaw()[2], Pose_x, Pose_y,Rads)
                
                pose_angle = imu_cleaner(imu.getRollPitchYaw()[2])
                #print("pose_angle before 360 conversion is :" + str(round(pose_angle,4)))
                if pose_angle > 359:
                   pose_angle = 360- pose_angle
                   
                #print("pose_angle after 360 conversion is :" + str(round(pose_angle,4)))
                #print("Pose in straight motion at time " + str(totaltime) + " is :" + str(round(Pose_x,2)) +" , "+str(round(Pose_y,2)) +" , "+str(round(pose_angle,2)))
                
                #if T[m] <= 32.00 :
                if round(totaltime,1) == T[m]:

                    print("Pose in straight motion at time " + str(T[m]) + " is :" + str(round(Pose_x,2)) +" , "+str(round(Pose_y,2)) +" , "+str(round(pose_angle,2)))
...
def updatePose(Imu, Pose_x, Pose_y,Rads):
    Radsleft = Rads[2] - Rads[0]
    RadsRight = Rads[3] - Rads[1]
    angularleft = ((Radsleft / 32) * 1000)
    angularright =((RadsRight / 32) * 1000)
    linearleft = angularleft * (1.6/2)
    linearright = angularright * (1.6/2)
                                
   
    linearvelocity= (linearleft + linearright)/2
    
    distance = linearvelocity * 32/1000
    
    
    Pose_x = Pose_x + distance * (math.cos(Imu))
    Pose_y = Pose_y + distance * (math.sin(Imu))
...
def updatePoseCircular(Imu, Pose_x, Pose_y,Rads):
    condition_angle = imu_cleaner(Imu)
    if condition_angle == 360:
        condition_angle = 0
    ...
    if condition_angle> 0 and condition_angle < 90:
   
        Pose_x = Pose_x + distance * (math.cos(Imu))
        Pose_y = Pose_y + distance * (math.sin(Imu))
        
    if condition_angle> 90 and condition_angle < 180:
   
        Pose_x = Pose_x + distance * (math.cos(Imu))
        Pose_y = Pose_y + distance * (math.sin(Imu))
        
    if condition_angle> 180 and condition_angle < 270:
    ...
    if condition_angle> 270 and condition_angle < 360:
...
T = [1.00,2.00,3.00,3.50,5.00,6.20,9.00,10.00,11.00,13.00,15.00,16.00,17.20,18.30,20.00,21.00,23.00,24.00,25.00,28,28.5,30.00,30.9,31.1,33.00]
robotPose(A,T)
